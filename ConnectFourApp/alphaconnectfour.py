...
def ai_simple_move(game_board, player):
    otherplayer = 1 if player == 2 else 2  # using the assumption there are 2 players

    # if there are no availble moves then return None
    ...
    other_player_winning_moves = game_board.get_winning_moves(otherplayer)
    if other_player_winning_moves:
        return random.choice(other_player_winning_moves)

    # else just select randomly, with a preferance for the middle
